...
// router.post('/api/download/stream',(res,req) => {
...
//         res.rawListeners(`data: ${data.toString().replace('/\n/g, \ndata : ')}\n\n`);
//     })
//     child.stderr.on('data', chunk => {
//         res.rawListeners(`data: [stderr] ${data.toString().replcae(/\n/g, '\ndata : ')}\n\n`);
//     })
//     child.on('close', code => {
//         res.write('data : [done]\n\n');
//         res.end();
...
//             return res.status(500).json({ error: stderr || error.message });
//         }
//         res.json({ result: stdout.stdout.trim().replace(/\n/g, '<br>')});
//     })
