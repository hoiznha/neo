...
from youtube_transcript_api._rrors import NoTranscriptFound, TranscriptDisabled
...
def divide_text(text, token_num):
    ...
    divide_text_list = textwrap.wrap(text, width=divde_width)
...
def summarize_youtube_video(video_url, selected_lang, trans_method):
    ...
    st.write(f'[제목] {yt_title}, [길이(분:초)] {video_info["duration"]}')
    ...
    divide_final_summary = my_text_sum.summarize_text_final(summaries, lang)
...
clikced_for_clear = st.sidebar.button('URL 입력 내용 지우기', on_click=button_callback)

yt_lang = st.sidebar.radio("유튜브 동영상 언어 선택", ['한국어','영어'], index=1, horizontal=True)

if yt_lang =='영어':
    trans_method = st.sidebar.radio('번역 방법 선텍', ['OpenAI','DeepL'], index=1, horizontal=True)
else:
    trans_method=''
