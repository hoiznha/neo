...
print('\n # fillna()')
print(myframe.fillna(0), inplace=False) # 0으로 채우기

print('\n inplace=False 사용시 원본 데이터 변동 없음')
print(myframe)
print('--'*50)

print('\n inplace=True 사용 시 원본 데이터 변동 있음')
print('\n # fillna()')
print(myframe.fillna(0), inplace=True) # 0으로 채우기
print('--'*50)
print(myframe)
print('--'*50)
...
myframe.fillna(mydict, inplace=True) # 0으로 채우기
print(myframe)
print('--'*50)

myframe.loc['박영희',['국어']]= np.nan
myframe.loc['김철수',['영어']] = np.nan
myframe.loc['홍길동',['수학']] = np.nan
print(myframe)
print('--'*50)
