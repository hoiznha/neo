...
#     """ Get the current price for a stock tocker. """
...
#     emails = {'123':'alice@exmaple.com', '456':'bob@example'}
...
#             file_content = f.read()
#     else:
#         file_content_resource = await ctx.read_resource(file_url)
...
#     await ctx.info(f'Finished processing {file_url}')
...
#         base, ext = os.path.splittext(os.path.basename(path))
...
#     """
#     load an image from disk the specified path(supports file:///),
#     save it as '<original_filename>_loaded.<extension>',
...
#     """Generate a short peom and about the given topic."""
...
#     """Summarize a document using server-side LLM capabilites.
#     Args:
#         document (str) : Either a resource URI (e.g., "system://docs/example.txt") 
#         or the actual document content.
...
#         model = "gpt-3.5-turbo",
#         messages = [
#             {'role':'system', 'content':f'You are an expoert summarizer. Create a concise summary.'},
#             {'role':'user','content':f'Summarize the following documnet : {document}'}
#         ]
#     )
#     return response.choices[0].message.content
